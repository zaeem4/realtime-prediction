...
if f.read() == "YES":
    ...
    class Deep_Evolution_Strategy:
        ...
        def train(self, epoch=100, print_every=1):
            ...
            logging.info("time taken to train:", time.time() - lasttime, "seconds")
    ...
    #         repair=True,
    #     )
    #     data.to_csv(f"dataset/GLD.csv")
    # except:
    #     logging.info(f"An exception occurred while GLD")


    # Loading initialdata


    logging.info("Reading Initial Dataset")

    df = pd.read_csv("dataset/GLD.csv")
    ...
    with open("model-rec.pkl", "rb") as fopen:
    ...
    logging.info("Downloading last 7 days data")
    ...
    try:
        ...
        logging.info(f"Error in downloading data of 7 days {ex}")
    ...
    logging.info("Starting with 7 days data")
    if len(real_trend) > 0 and len(volume):
        for i in range(len(real_trend)):
            logging.info(agent.trade([real_trend[i], volume[i]]))
    else:
        ...
        try:
            try:
                data = yf.download(
                    tickers="GLD",
                    interval="1h",  # trading interval
                    period="6h",  # time period
                    prepost=False,  # download pre/post market hours data?
                    repair=True,
                )
            except ex:
                logging.info(f"Error Downloading last 6 hours data {ex}")
                continue
            ...
            if len(real_trend) > 0 and len(volume):
                logging.info(
                    agent.trade([real_trend[len(real_trend) - 1], volume[len(volume) - 1]])
                )
                time.sleep(60 * 69)
                logging.info("1 hour passed")
            else:
                logging.info("data not found in while 1")
        except ex:
            logging.info(f"Error in while loop{ex}")
else:
    print(f"AUTO RUN IS NOT YES")
