...
with open("../models/model-gold-f-07.pkl", "rb") as fopen:
...
while 1:
    try:
        try:
            ...
            # data = data.dropna()
            data.to_csv(f"../timeseries/GC=F.csv")
        except Exception as e:
        ...
        if len(real_trend) > 0 and len(volume) > 0:
            ...
            logging.warning([real_trend[len(real_trend) - 1], volume[len(volume) - 1]])
        else:
