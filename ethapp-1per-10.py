...
logging.basicConfig(
    filename="../logs/ethapp-1per-10.log",
...
class Agent:
    ...
    def trade(self, data):
        ...
        if action == 1 and self._scaled_capital >= close * 0.01:
            self._inventory.append(close * 0.01)
            self.real_inventory.append(real_close * 0.01)
            self._scaled_capital -= close * 0.01
            self._capital -= real_close * 0.01
            return {
                "status": "buy 1 unit, cost %f, inventory %d"
                % (real_close * 0.01, len(self._inventory)),
                "action": "buy",
                "balance": self._capital,
                "timestamp": str(datetime.now()),
            }
            ...
            try:
                invest = (
                    ((real_close * 0.01) - (scaled_bought_price * 0.01))
                    / (scaled_bought_price * 0.01)
                ) * 100
            ...
            return {
                ...
                % (real_close * 0.01, len(self._inventory)),
                "investment": invest,
                "gain": real_close * 0.01 - (scaled_bought_price * 0.01),
                "balance": self._capital,
                "action": "sell",
                "timestamp": str(datetime.now()),
            }
...
initial_money = np.max(parameters[0]) * 1

logging.warning(f"Initial Money: {initial_money}")

# Loading Model

with open("../models/model-stock-f-06.pkl", "rb") as fopen:
...
# logging.warning("Downloading last 5 days data of 1 hour")

# try:
#     data = yf.download(
#         tickers="BTC-USD",
#         interval="1h",  # trading interval
#         period="5d",  # time period
...
#     data = data.dropna()
# except Exception as e:
#     logging.warning(f"Error in downloading data of 5 days {e}")
