...
try:
    ...
    data.to_csv(f"../dataset/GC=F.csv")
except Exception as e:
...
df = pd.read_csv("../dataset/GC=F.csv")
df = df.dropna()

real_trend = [i * 0.01 for i in df["Close"].tolist()]
parameters = [[i * 0.01 for i in df["Close"].tolist()], df["Volume"].tolist()]
...
with open("../models/model-gold-f-1per-07.pkl", "rb") as fopen:
...
logging.warning("Downloading last 3 day data of 1 hour")

try:
    data = yf.download(
        tickers="GC=F",
        interval="1h",  # trading interval
        period="3d",  # time period
        prepost=True,  # download pre/post market hours data?
        repair=True,
    )
    ...
    logging.warning(f"Error in downloading data of 3 day {e}")

logging.warning("Starting with 3 day data")

real_trend = [i * 0.01 for i in data["Close"].tolist()]
volume = data["Volume"].tolist()

if len(real_trend) > 0 and len(volume) > 0:
    for i in range(len(real_trend) - 1):
        logging.warning(agent.trade([real_trend[i], volume[i]]))
        logging.warning([real_trend[i], volume[i]])
else:
    logging.warning("data not found")

time.sleep(60 * 10)

logging.warning("Threadpool Started")
...
while 1:
    try:
        try:
            ...
            # data = data.dropna()
            data.to_csv(f"../timeseries/GC=F-1per.csv")
        except Exception as e:
        ...
        if len(real_trend) > 0 and len(volume) > 0:
            logging.warning(
                agent.trade(
                    [real_trend[len(real_trend) - 1] * 0.01, volume[len(volume) - 1]]
                )
            )
            logging.warning(
                [real_trend[len(real_trend) - 1] * 0.01, volume[len(volume) - 1]],
